                        ...
                        <tr>
                            ...
                            <td width="20%" class="t_color">${boardDocDTO.noticeYn}</td>
                         	...
                         	<c:forEach items="${boardDocDTO.fileList}" var="file">
		                        <tr>
		                          	<td width="8%">첨부파일</td>
		                            <td colspan="7" class="txtCut alignLeft"><a href="${_ctx}/file/downloadFile.ssol?docId=${file.docId}&fileSno=${file.fileSno}">${file.orgFileName}</a></td>
		                        </tr>
                         	</c:forEach>
                        <tr>
                            <td colspan="10" class="alignLeft">${boardDocDTO.boardContents}</td>
                        </tr>
                    </tbody>
                </table>
                <div class="btnSet">
                    ...
                    <a href="${_ctx}/board/notice/write.ssol?mapId=${boardDocDTO.mapId}" class="disPB btnBase">글쓰기</a>
...
<%--                     <input type="hidden"  id="b" value="${userDTO.userId == boardDocDTO.userId}"/> --%>
                    	<a href="${_ctx}/board/notice/edit.ssol?mapId=${boardDocDTO.mapId}&docId=${boardDocDTO.docId}" class="disPB btnBase">수정</a>
                    	<a href="${_ctx}/board/notice/remove.ssol?mapId=${boardDocDTO.mapId}&docId=${boardDocDTO.docId}" class="disPB btnBase">삭제</a>
                		</c:if>
                </div>
	...
	<form id="frmList"  name="frmList" action="${_ctx}/board/notice/list.ssol" method="get" >
